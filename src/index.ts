...
function getApiName(){
  ...
  else if(isEthereumApiFromTrustWallet()){
...
class SmartyPayMetamask implements Web3Api {
  ...
  async connect() {
    ...
    this.curAddress = accounts && accounts.length > 0?
        Web3Common.getNormalAddress(accounts[0])
        : undefined;
    ...
    getEthereum().on('accountsChanged', (accounts: any) => {
      ...
      const newAddress = accounts && accounts.length > 0 ?
          Web3Common.getNormalAddress(accounts[0])
          : undefined;

      if(this.curAddress === newAddress)
        return;
